...
def saveHistogram(values, rejectedAstrometryIndices, rejectedFWHMIndices, rejectedBackgroundIndices, rejectedCalibrationFactorIndices, title, xLabel, imageName, valueForMeanVerticalLine=None, valueForStdVerticalLines=None, valuesForMultipleVerticalLines=None):
...
# 3.- Retrieve the calibration factor(s) to apply, and build one array of
# [filename, factor] pairs - the actual per-frame value each mode applies,
# whether that's a single global number, that frame's own night's number,
# or that frame's own individually-computed number.
...
if (calibrationFactorScope == "global"):
    ...
    # Identify frames that are outside the Nsigma limit of the distribution
    ...
    arrayWithFactorToApply = [[x[0], commonCalibrationFactorValue] for x in totalCalibrationFactors]
    ...
    # One shared pass: for each frame, find its night once, and use it both
    # to build the applied-factor array and to check it against its own
    # night's mean/std (rather than a single global mean/std, since factors
    # can genuinely differ night to night).
    nightToOwnFactors = {}   # night -> list of that night's frames' own individual factors
    frameNight = {}          # filename -> night
...
else:  # individual - each frame keeps its own factor, nothing to compute
    arrayWithFactorToApply = totalCalibrationFactors
 
 
# 4.- Apply the calibration factor(s) built above
